...
export default function TopStrip({ score, elapsed = 0, onReset, onBack, level }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        >←</button>
        <div>
          <div style={{ fontSize: 9, letterSpacing: 3, color: '#7ec8ff', fontWeight: 700, opacity: 0.8, marginBottom: 2 }}>
            {level?.name?.toUpperCase() ?? 'MISSION'}
          </div>
          <div style={{ fontSize: 20, fontWeight: 800, letterSpacing: -0.4, lineHeight: 1.1 }}>
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
        ...
        <Stat label="SCORE"  value={String(score).padStart(3, '0')} highlight={score < 60} />
        <Stat label="RATING" value={<StarRating score={score} />} />
        <button onClick={onReset} style={ctrlBtn}>Retry</button>
...
function Stat({ label, value, highlight }) {
  return (
    <div style={{ textAlign: 'right' }}>
      <div style={{ fontSize: 9, letterSpacing: 1.5, fontWeight: 700, color: 'rgba(255,255,255,0.45)' }}>
        {label}
      </div>
      <div style={{
